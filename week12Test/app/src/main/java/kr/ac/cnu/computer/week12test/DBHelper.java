...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Message selectOne(long insertedId) {
        ...
        Cursor cursor = db.query("message", new String[]{"id", "contents", "register_date", "message_type"},
                "id = ?", new String[]{String.valueOf(insertedId)}, null, null, null);
    ...
    public List<Message> selectAll() {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("message", new String[]{"id", "contents", "register_date", "message_type"},
                null, null, null, null, "register_date");
        while (cursor.moveToNext()) {
            int id = cursor.getInt(0);
            String contents = cursor.getString(1);
            String registerDate = cursor.getString(2);
            int code = cursor.getInt(3);
            messages.add(new Message(id, contents, registerDate, MessageType.of(code)));
        }
        return messages;
    }
...
//        values.put("register_date", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss", Locale.KOREA).format(new java.util.Date()));
//        values.put("message_type", String.valueOf(MessageType.RIGHT_CONTENTS));
